...

{
  ...
  assert.equal(battles.startBattle(state), true);
...
// 지도에 무리가 없는 폴백 습격(접근 경로 없음)은 전투를 열지 않고 즉시 판정으로 넘긴다
{
  ...
  assert.equal(battles.startBattle(state), false);
...
// 밸런스: 전투 전체 루프의 승률이 기존 즉시 판정 공식 defense/(defense+power)와 일치해야 한다
{
  ...
  const power = 60;
  const defense = computeDefense(state); // 중심지 5 + 민병 3×12 = 41
...
// 징집 즉시 기존 작업 경로를 버리고 임시 민병으로 바뀌어 전선으로 향해야 한다
{
  ...
  assert.equal(battles.startBattle(state), true);
  assert.equal(defender.job, 'militia');
  assert.equal(defender.path.length, 0);
  assert.equal(defender.targetId, null);
  assert.deepEqual(state.battle.draftedJobs, [{ id: defender.id, job: 'watchman' }]);
