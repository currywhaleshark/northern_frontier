...
assert.match(atlasSource, /requiredAssets\.every\(asset => asset\.status === 'loaded'\)/,
  'atlas readiness is derived from explicit required asset states');
assert.doesNotMatch(atlasSource, /loaded\s*>=\s*\d+/, 'atlas readiness has no asset-count magic number');
