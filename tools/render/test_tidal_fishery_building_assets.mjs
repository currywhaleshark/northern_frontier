...
const hdPng = readFileSync(new URL('public/assets/tidal-fishery-building-hd-v1.png', ROOT));
...
assert.equal(hdPng.toString('ascii', 1, 4), 'PNG', '어살터 HD 아틀라스가 PNG여야 한다');
...
assert.equal(manifest.display.hdSourceScale, 8);
...
assert.match(atlasSource, /loadAtlasAsset\(TIDAL_FISHERY_BUILDING_HD_SHEET\.src/,
  '게임 아틀라스가 어살터 HD 시트를 별도로 로드해야 한다');
assert.match(atlasSource, /p\.highDefinition && tidalFisheryBuildingHdSheet/,
  '최대 줌과 스프라이트 스튜디오는 어살터 HD 시트를 골라야 한다');
