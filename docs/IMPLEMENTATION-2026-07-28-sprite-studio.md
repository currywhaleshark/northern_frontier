...
> **계획 상태:** 일부 완료
> **상태 갱신:** 2026-08-02 — P0~P6은 완료했으며, 지표 광상 인접 채광에 맞춘 대상 방향 미리보기 P7을 진행한다.
...
- [ ] `residentWorkStances`가 서 있는 지형과 별도로 실제 작업 대상 지형·좌표를 받을 수 있다.
- [ ] 인접 채광 중인 광부는 `miner@rock` 앵커를 계속 적용하고 좌우 광상 방향을 바라본다.
- [ ] 탭 B에서 광부는 중앙 통행 칸, 광상은 선택한 인접 칸에 표시되며 네 방향을 바꿔 확인할 수 있다.
- [ ] 기존 벌목꾼·약초꾼의 같은 칸 작업 문맥과 저장된 앵커 데이터는 그대로 유지한다.
