...
> **계획 상태:** 일부 완료
> **상태 갱신:** 2026-08-02 — P8에서 인접 광부와 노두의 방향별 가림 순서를 게임·스튜디오에 공통 적용할 예정이다.
...
- [ ] 광부가 노두 위쪽 칸에서 작업하면 실제 깊이 관계대로 노두 뒤에 그려진다.
- [ ] 광부가 노두 좌우 칸에서 작업하면 큰 노두에 깔리지 않고 노두 앞에 그려진다.
- [ ] 광부가 노두 아래쪽 칸에서 작업하면 기존 행 정렬대로 노두 앞에 그려진다.
- [ ] 게임 렌더러와 탭 B가 동일한 순수 정렬 계산을 사용하며 네 방향 회귀가 이를 고정한다.
