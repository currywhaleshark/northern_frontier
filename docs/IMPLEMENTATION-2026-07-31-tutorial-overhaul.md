...
> **계획 상태:** 완료
> **상태 갱신:** 2026-08-02 — R8에서 겨울 점검 비축량을 권장선으로 바꾸고, 첫 겨울을 놓친 v7 진행 저장도 둘째 해 달력으로 복구하도록 길잡이 v8·회귀를 갱신했다.
...
「조정은 첫 해의 정착을 지켜보고 있습니다. 이듬해부터는 소출의 일부를 세공으로 바쳐야 합니다.」
...
- 일반 새 게임 첫날 로그에 「조정은 첫 해의 정착을 지켜보고 있습니다. 이듬해부터는 소출의 일부를 세공으로 바쳐야 합니다.」가 서고,
...
당시 제약 확인: R5 구현 시점에는 채광장(mine)이 보(堡) 승격 전용이었다. 이후 채집 영역 설계 G3(2026-08-02)가 이 결정을 대체해 채광장을 촌 단계 거점으로 내리고 배정 없는 지표 채광을 제거했다. 대장간·장터도 촌 등급 건설 가능하다.
...
- [x] 13 광물: 광맥 레이어 켜기 + 채광장 건설 + 채광장 작업 슬롯 배정 + 영역 안 지표 노두에서 돌·철 채굴 (G3 후속 반영)
...
**R5 재정정 (2026-08-01 사용자, 역사 기록):** 당시에는 채광장 잠금을 풀지 않고 방랑 채광을 유지했다. **G3 후속 정정 (2026-08-02 사용자):** 채집 직업 전체의 거점 배정을 필수화하면서 이 결정을 다시 뒤집었다. 채광장은 촌 단계, 채광꾼은 완공 채광장 작업 슬롯 배정 필수다. 마을 옆 석재·철 노두 보장은 첫 채광장 영역을 여는 시작 자원으로 유지한다.
...
| 13 | `minerals` | 노두와 채광 | 광맥 탭 1 · 채광장 1 · 채광장 배정 1 · 돌·철 `mineralsMinedGoal`(8) | 플래그 `oreToggled` / 완공 수 / 작업 슬롯 배정 수 / **캐낸 누계**(`mineralsMined`) |
...
G3 이후 채광장(`mine`)은 촌 단계 채집 거점이다. 채광꾼은 완공 채광장 작업 슬롯에 배정되어야 하며, 해당 반경의 지표 노두(`terrain === 'rock' ∧ mineralRemaining > 0`)만 캐 채광장에 부린다. 스텝은 **광맥 탭 → 채광장 건설 → 작업 슬롯 배정 → 돌·철 채굴**로 성립한다.
...
**목표치 산정**: `mineralsMinedGoal = 8`을 유지한다. G3 완주 회귀는 길잡이 시드의 마을 인근 노두 옆에 채광장을 세우고 작업자를 배정해 실제 채굴 누계로 스텝이 닫히는 것을 확인한다.
...
## R6 — 연례 세공 파발 삽화 창 (2026-08-01)

- 담당: `GPT-5 Codex 주 에이전트 / high`
- [x] 둘째 해부터 매해 봄, 세공 품목·수량·겨울 첫날 기한을 확인형 사건 창으로 표시
- [x] 기존 사건 창이 있으면 덮어쓰지 않고 `tributeAnnouncementPendingYear`에 기억했다가 해소 직후 표시
- [x] 첫 파발 삽화 창이 초회 세공 길잡이를 겸하고, 가죽옷 요구 시 가죽공방 카드만 후속 표시
- [x] 수거 삽화와 구분되는 봄 파발 도착 수묵담채화 `court-tribute-dispatch-v1.png` 생성·연결
- [x] `test_court_tribute.mjs`·`test_tutorial_scenario.mjs` 표적 회귀 통과

## R7 — 첫 벌목장 건축가 안내 순서 보정 (2026-08-02)

- [x] 「사람과 일」 목표를 `벌목장 터 → 건축가 → 벌목장 완공 → 작업 슬롯 배정` 순서로 분해
- [x] 벌목장 터를 잡은 직후 `job-plus-builder`를 가리키고, 그다음 벌목꾼 상세 배정을 안내
- [x] 「집과 장작」의 건축가 안내는 앞 단계의 배정을 유지하되, 다른 직업으로 돌렸을 때만 나타나는 구제 힌트로 전환
- [x] 목표 의미 변경에 따라 `TUTORIAL_SCENARIO_VERSION`을 7로 갱신하고 17단계 완주 회귀 통과

## R8 — 첫 겨울 준비 지연 복구 (2026-08-02 사용자 플레이테스트)

플레이테스트에서 식량 30일분을 첫 겨울 전까지 채우지 못해 `stocktake`에 남았고, 이듬해 봄에
목표를 채워도 `winter`의 진행값이 0/10으로 돌아가 다음 겨울까지 시나리오가 멈추는 문제가 확인됐다.
완주 회귀가 식량·장작을 매일 보충해 이 실패 경계를 가리고 있었다.

- [x] 식량 30일분·땔감 24일분은 겨울 점검 패널의 **넉넉함 권장선**으로 유지하되,
      `stocktake` 완료 조건에서는 제외한다. 필수 학습은 겨울 점검 패널을 한 번 여는 것이다.
- [x] 점검을 열지 못한 채 첫 겨울이 시작되면 `stocktake`를 자동 통과하고, 부족 항목을 계속
      보충하라는 중요 경고를 정확히 한 번 남긴다. 자원 지급·달력 정지·날짜 되감기는 하지 않는다.
- [x] `winter` 진행값은 첫 겨울 10일째를 지난 뒤 이듬해 봄에도 완료값을 유지한다. 따라서
      늦은 진행 저장이 다음 겨울까지 기다리지 않고 둘째 해 `tribute` 단계로 복귀한다.
- [x] 통제 혹한은 첫해 겨울에만 발화한다. 첫 겨울을 놓친 저장에서 둘째 겨울에 뒤늦게
      재생하지 않는다.
- [x] 목표 의미 변경으로 `TUTORIAL_SCENARIO_VERSION`을 8로 올리되, 스텝 순서가 같은 v7 저장은
      로드 시 v8로 호환 승격해 사용자의 진행을 보존한다. 더 오래된 비호환 판은 기존처럼 해제한다.
- [x] `test_tutorial_scenario.mjs`에 `부족한 채 겨울 진입 → 자동 생존 단계 → 둘째 해 봄 tribute 복구`,
      경고 1회, 봄 눈금 걸쇠, 혹한 비재생, v7 저장 호환 승격 회귀를 추가한다.

**검증 결과:** `node tools/game/test_tutorial_scenario.mjs` 통과, core `npm run test:game` 77/77 통과,
`npm run build`의 타입 검사·프로덕션 빌드 통과.
