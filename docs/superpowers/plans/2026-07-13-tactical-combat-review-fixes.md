...
> **계획 상태:** 완료
> **상태 갱신:** 2026-07-30 — 기존 범위와 함께 무장 종류와 무관한 사냥꾼 준비 매복 회귀도 보정했다.
...
**후속 회귀 보정 (2026-07-30):** 사냥꾼이 창·각궁·조총을 들면 전술 그룹 `kind`가 무기
병종으로 바뀌더라도 `role`과 capability를 기준으로 준비 매복을 판정한다. 따라서 수동·자동
배치 모두 접근로에서 첫 교전 매복이 켜진다. `test_tactical_battle.mjs`로 창 장비 사례를 검증했다.
