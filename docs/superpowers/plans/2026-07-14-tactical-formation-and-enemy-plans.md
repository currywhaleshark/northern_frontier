...
- [ ] 구역 내부를 `적 후열│중열│전열│교전선│아군 전열│중열│후열` 세로 칸으로 렌더링하고, 같은 열의 부대는 위아래로 쌓는다. 공격전은 방향 반전(기존 `assault` 클래스 활용).
- [ ] 화면 밀도 대책: 포커스 구역(`currentZoneId`)만 3열 확대, 비포커스 구역은 기존 압축 표시 유지.
- [ ] 부대 칩에 열 표시를 `전열/중열/후열`로 갱신. 배치 단계에서 열 이동 버튼 3값 지원(`onSetFormationLine` 경로 재사용).
- [ ] 시각 검증: 미리보기로 배치→교전→후방 급습 연출까지 확인.
...
- [ ] 급습 발생 구역에서 가능한 대응을 명령 조합으로 노출한다: 중열 `reinforceRear` / 전열 `redeploy`(→후열, 정면 압박 증가 감수) / 원거리 자체 대응(후방 교전에 자동 편입되되 근접 노출 큼) / 무대응. 신규 시스템을 만들지 않고 2.1·2.2의 조합으로 성립시킨다.
- [ ] `chooseDefaultTacticalCommands`: 급습 발생 시 중열 근접 예비대가 있으면 `reinforceRear`를 추천 명령으로 채운다.
