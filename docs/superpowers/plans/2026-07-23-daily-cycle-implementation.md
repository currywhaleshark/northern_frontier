...
- **(2차 수정) 이동 속도 유지 + 하루 밀도 상승**: 슬로모션(×6 비례 서브틱 연장) 대신
  ...
  구현은 A안(SUBTICKS 72, 667ms 서브틱 유지 — Fable 권고) vs B안(16 유지 + 다중 타일
  이동·폴리라인 보간) 중 Codex 선택. 대역 비율은 2:8:3:3 유지
...
| 리스크 | 완화 |
|---|---|
| 성능 (+50% 서브틱) | 취침 스킵 + §5 게이트 + 미달 시 대역 축소 |
| 밸런스 회귀 | 노동 8서브틱 구조 보장 + 생산 총량 불변 테스트 + 장기 시뮬 회귀 |
| atlas.ts 충돌 | 선행 브랜치 병합 후 착수 (§7-1) |
| 저장 호환 | 단일 마이그레이션 + phase/path/carry 보존 + 첫 M1 틱에서 대역 불일치만 정리 |
