...
> **계획 상태:** 일부 완료
> **상태 갱신:** 2026-07-29 — 가축·의원·귀순병·기마병은 구현됐으나 팽배수 등 후속 범위가 남아 있다.
...
> 2026-07-17 구현 기록: 귀순 주민은 `origin`을 가진 일반 주민으로 생성되며, 자연 이주·취약 외교 거점·홀라온 군마 사건의 세 경로를 연결했다. 니마차 매복/정찰, 홀라온 기마 숙련 표식, 조정 이탈병 조총 보정이 개인 전투 스냅샷과 전술 그룹에 유지된다. 군마는 사육과 해금까지만 열었고 기마병 편성은 후속 전투 유닛 단계에 남겼다. 2026-07-18에는 실록의 두만강 유역 여진인 음차 인명으로 전용 이름풀을 추가해, 북방 귀순 주민이 조선 성명으로 생성되지 않게 했다(조정 이탈병은 기존 조선식 이름 유지).
...
- [x] 방어전: 구역 이동(`advance`/`fallback`)·열 재배치(`redeploy`) 라운드 페널티 면제 — "이동이 공짜인 예비대". 후방 급습 대응 기동이 기마의 존재 이유다. 돌격 보정은 소폭(정적 농성에서 과강화 방지), 목책 안 수성에는 보정 없음.
- [x] 추격: 승리·궤주 시 `recoverRoutedLoot` 회수율 상향(기존 50% 고정 → 기마 수에 따라 상한까지 가산)과 도주 적 추가 처치. 원정 행군 속도 가산.
- [x] 고정 시드 실측: 기마 유무가 "전투 결과 뒤집기"가 아니라 "피해 통제·회수율 개선"으로 나타나는지 확인한다.

> 2026-07-17 F2 구현 기록: 실제 `mountAssignments`를 개인 전투 스냅샷과 방어·토벌·사냥 전술 그룹까지 보존하고, 홀라온 출신의 기마 숙련과 현재 탑승 상태를 분리했다. 기마대는 이동 명령 중 보병 페널티 대신 일반 수성 수준 전력을 유지하며 중열에서 후방 급습에 대응한다. 야전 창 돌격은 +0.10만 받고 목책 구역에서는 보정이 없으며, 정지 수성은 보병과 동일하다. 적 궤주 시 생존 기마 1명당 약탈 회수율 +4%p(50~75%), 도주 적 추가 처치(기마당 0.4명, 최대 6명), 원정 편성 비율에 따라 행군 속도 최대 +30%를 적용했다. 32개 고정 시드의 동일 후퇴 교전에서 보병 누적 피해 21명 대비 기마 15명, 적 전력 점유 합 26.49 대비 18.02로 피해 통제 효과를 확인했고 정지 수성 배율은 동일했다. 기마 관련 직접 테스트는 모두 통과했으며 전체 게임 테스트는 병행 구현 중인 `cemetery` 빌드 메뉴 누락 1건을 제외한 80/81개가 통과했다.
