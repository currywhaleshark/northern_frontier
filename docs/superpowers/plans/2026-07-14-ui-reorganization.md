...
> **계획 상태:** 완료
> **상태 갱신:** 2026-07-29 — 건설 드로어·도킹 창·선택 바·통합 로그와 안정화를 완료했다.
...
- [x] 시각 검증: 미리보기로 그룹 접힘/팝오버/별표 추가·제거/low 강조를 확인하고 스크린샷을 남긴다.
  ...
  - 2026-07-16 후속 검증: 1280×720에서 7개 그룹의 접힘 상태, 자재 팝오버, 도구 별표 추가·제거, 도구 2.5에서 자재 칩과 숨은 도구 행의 low 강조를 확인했다. 통합 로그가 팝오버 일부를 가리는 레이어 충돌을 발견해 팝오버를 로그보다 위로 올리고 회귀 테스트를 추가했다. 전체 게임 테스트 72개·프로덕션 빌드·`git diff --check`가 통과했다.

![U1 자원 그룹 시각 검증 — 부족한 도구의 그룹·행 강조와 별표 노출](./assets/ui-reorganization-u1-resource-popover-1280x720.png)
...
**Files:**
- Modify: `src/App.tsx`, `src/styles/global.css`
- Modify: `tools/game/test_selection_context_ui.mjs`, `tools/game/test_sidebar_removal_ui.mjs`
- Create: `tools/game/test_minimap_overlay_layout_ui.mjs`, `docs/superpowers/plans/assets/ui-reorganization-final-1280x720.png`

- [x] 미니맵을 우하단으로 이전(좌상단 로그·우상단 경고·우측 도킹 스트립·하단 컨텍스트 바와 겹치지 않는 마지막 빈 모서리). 하단 컨텍스트 바가 열릴 때 미니맵이 가려지지 않는지 확인.
- [x] 최종 오버레이 배치도(좌상단 로그 / 우상단 경고 / 우측 도킹 / 하단 건설+컨텍스트 / 우하단 미니맵)를 이 계획서에 스크린샷과 함께 기록해 마감한다.

최종 배치 (2026-07-16): 좌상단에는 통합 로그, 우상단에는 경보, 우측에는 관리 도킹 스트립과 열린 창, 좌하단에는 건설 메뉴를 둔다. 우하단은 미니맵과 선택 컨텍스트를 하나의 세로 스택으로 묶어 컨텍스트가 열리면 미니맵이 그 위로 올라가며, 도킹 창이 열리면 스택 전체가 창 왼쪽으로 이동한다. 도킹 창과 스택 사이는 8px로 유지하고, 빈 스택 영역은 지도 입력을 통과시킨다.

![최종 오버레이 배치 — 1280×720, 건물 선택 및 사건·기물함 도킹 창 동시 표시](./assets/ui-reorganization-final-1280x720.png)

검증 메모 (2026-07-16): 1280×720에서 기본 상태, 건물 선택 컨텍스트 표시, 사건·기물함 도킹 창 동시 표시를 확인했다. 미니맵은 선택 컨텍스트와 8px 간격으로 세로 배치되고, 열린 도킹 창과도 8px 간격을 유지하며, 좌하단 건설 메뉴 및 좌상단 로그와 겹치지 않는다.
