...
- [x] 건설 드로어와의 공존 규칙: 컨텍스트 바가 나타나면 드로어는 접힌다(하단 공간 경합 방지). 반대로 건설 배치를 시작하면 선택이 해제된다(현행 동작 유지 확인).

검증 메모 (2026-07-16): 타일·건물·주민 선택 문맥과 기존 건물 행동을 하단 `SelectionContextBar`로 이전했다. 선택 시 건설 드로어가 닫히고, 건설 카테고리 또는 배치를 시작하면 선택이 해제된다. 확장 건설 드로어는 좌하단, 선택 컨텍스트는 우하단의 약 20vw 폭 소형 패널로 분리했다. 1280×800·1920×1080에서 도킹 창과 컨텍스트의 경계가 겹치지 않으며, 긴 주민 정보는 패널 내부에서만 스크롤됨을 확인했다.
