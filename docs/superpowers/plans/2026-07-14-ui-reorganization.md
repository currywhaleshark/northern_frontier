...
- [x] 하단 상시 아이콘 바(건설 카테고리: 주거/생산/농사/방어/특수) → 클릭 시 해당 카테고리 드로어가 위로 펼쳐진다. 카드는 스프라이트·이름만 노출하고 설명·비용·공기·티어 잠금 사유는 hover/focus 툴팁과 접근성 설명으로 제공한다. 기존 메뉴가 숨기던 잠금 건물도 포커스 가능한 `aria-disabled` 카드로 유지한다.
