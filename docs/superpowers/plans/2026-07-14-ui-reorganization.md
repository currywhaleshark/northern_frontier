...
> **계획 상태:** 완료
> **상태 갱신:** 2026-07-30 — 자원 상세 행 줄바꿈 방지·수치 정수화·주민 로그 호칭까지 UI 안정화를 완료했다.
...
표시 보강 (2026-07-30): 내부 처리량 배율로 소수가 되는 운반꾼 적재량과 고유 무기 배율이
적용된 개인 방어 기여도는 UI에서 소수점을 버린 정수로 표시한다. 자원바·상세 자원·생산 비축도
같은 정수 표시 정책을 유지하며, 급수량·사료 소모처럼 의미 있는 소수는 고정 자릿수로 제한한다.
일반 주민이 등장하는 개인 로그·질병 사건에는 현재 직업 또는 성장 단계와 이름을 함께 표시한다.
네임드는 칭호가 포함된 기존 이름을 유지하고, 사망 당시 역할은 시신에 보존해 수습·안장 로그까지
같은 호칭을 이어 간다.
자원 상세 팝오버는 품목명·수량·부패량·별표를 한 행에 유지하고, 부패 표시가 있는 식량도
품목명이 글자 단위로 줄바꿈되지 않도록 내용 폭과 고정 열을 확보한다.
