...
> 최초 작성: 2026-07-20 새벽. 최종 갱신: 2026-07-20, Phase 9 혼잡 QA 라벨 수정 완료.
...
## 15. 2026-07-20 Phase 9 시뮬레이터 — 특수주민·강제 계책·16그룹 QA

Phase 9 프론트의 1280×720 밀도 QA를 재현할 수 있도록 방어전 시뮬레이터에 두 계약을 추가했다.

- `includeCombatSpecialResidents`는 전투 표현이 있는 특수주민 4명, 아라개(`jurchenWarrior`)·박돌개
  (`tigerHunter`)·단심(`uinyeo`)·사야카(`hangwae`)를 격리 상태에 추가한다. 각자의 정본 이름·성별·나이·직업과
  `special` ID를 사용하므로 기존 전술 명명조·치료반·개인 무기 규칙을 그대로 탄다.
- `enemyStratagem`은 `auto | none | EnemyStratagemId`다. 특정 계책은 자동 구매 결과의 첫 항목으로 강제되며
  중복 없이 최대 계책 수를 지킨다. `none`은 모든 계책과 후방 경로를 제거한다. 후방 우회와 경로 없음처럼
  서로 충돌하는 UI 선택은 자동으로 일관된 값으로 되돌린다.
- 설정 UI의 계책 목록은 `enemyStratagemDefinitions()`를 소비한다. 계책 ID·라벨·비용을 프론트에서 복제하지 않는다.

16그룹 재현 절차:

1. 마을 방어전 · 조정 토벌군 · `기존 조정 토벌군` 편제 · `방책 파괴조` 계책 · 특수주민 `4명 포함`을 고른다.
2. 아군 구성을 `지정` 기본값으로 두고 전투를 시작한 뒤 준비 단계를 실행한다.
3. 사야카·아라개·박돌개를 각각 `이름 있는 조 분리`하고, 2명 이상인 일반 조 하나를 1명 분리한다.
4. 이 상태는 지휘 가능 아군 10개 조다. 관군은 기존 5개 편제에서 파책 살수조가 분리되어 6개 조가 되므로
   총 16개 그룹이 된다. 자동배치 후에도 상한과 인원 총합을 보존한다.

계약 테스트는 이 절차를 도메인 함수로 그대로 실행해 10+6을 고정한다. 1280×720 실화면에서도 설정 패널은
가로 넘침 없이 내부 스크롤되고, 전술 무대와 하단 배치 카드는 고정 높이와 내부 스크롤을 유지했다. 브라우저
콘솔 경고·오류는 없었다. 실기기 포인터 드래그·터치는 사용자 실기기 세션 결과를 기다린다.

## 16. 2026-07-20 Phase 9 혼잡 QA 후속 — 키보드 통합·분견대 라벨

- Fable의 배치 카드 키보드 선택 수정 `215d19c`와 선행 혼잡 QA 커밋을 통합 브랜치에 ff 병합했다.
  잠기지 않은 카드도 Enter/Space가 만드는 `detail === 0` 클릭으로 선택되며, 포인터 드래그 훅의 클릭 경로와
  중복 실행하지 않는다.
- QA에서 발견한 `t_f75916d6` 라벨 버그를 수정했다. 이름 있는 조 분리 액션명은 계속
  `아라개의 조 분리`지만, 생성된 전투조 라벨은 병과를 보존해 `아라개의 창 수비병`으로 표시한다.
- 같은 코호트의 일반 분할조는 숫자 `1조·2조·3조` 대신 `갑조·을조·병조`를 쓴다. 현재 코호트 상한이
  3개이므로 세 서수가 전체 정상 범위를 덮고, 미래에 상한이 늘어날 경우에만 숫자 fallback을 사용한다.
- 방어전·사냥전 계약 테스트가 분할, 병합, 명명 조 특기 이전, 갑을병조 라벨을 함께 고정한다.
- 1280×720 실화면에서 아라개 분리 뒤 `창 수비병 갑조`와 `아라개의 창 수비병`, 추가 일반 분리 뒤
  `창 수비병 을조`를 확인했다. 콘솔 경고·오류는 없었다.
