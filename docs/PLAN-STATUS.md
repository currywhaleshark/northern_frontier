...
| [튜토리얼 개편 착수 지시문](IMPLEMENTATION-2026-07-31-tutorial-overhaul.md) | 완료 — R4 포함 | Claude Opus 단독 담당. M0~M6에 이어 플레이테스트 후속 R1 완료(시작 전원 무직·시작 건초 30·시나리오 중 자연 병자 잠금·일시정지 시작과 스텝 자동 일시정지), R2 완료(직업 배정 안내 이원화·첫 병자 스텝에 약초막과 약초꾼 편입·겨울 점검 문구 교정), R3 완료(목표 칩 소목표 진행도), R4 완료(첫 해 무세공·시나리오 10스텝·길잡이 출신 첫 세공 가죽옷 고정·세공 파발/가죽공방 길잡이 2모듈) |
