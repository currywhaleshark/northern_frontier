...
| [어선과 출어 설계](DESIGN-2026-08-02-fishing-boats.md) | 미착수 | 배무이터·어선 기물·포구 작업영역·어장 비축·실출어 F1~F3 설계. 새 게임 설정 S4(호수)·S5(해안) 지형에 종속 |
| [새 게임 설정 개편 설계](DESIGN-2026-08-01-new-game-setup.md) | 미착수 | 시작 설정창·지도 크기 3종·지역 타입 4종(평원/산지/호수/해안—자염 소금 자급)·자원 밀도 프리셋 S1~S5 설계 확정 |
...
| [튜토리얼 개편 구현계획](superpowers/plans/2026-07-31-tutorial-overhaul.md) | 완료 | M0~M6·R1~R7 반영. 첫 벌목장 터 직후 건축가를 먼저 배정하는 목표·코치 순서까지 보정. §9 테스트 7항목 전수 커버, §11 비범위는 후속 |
| [튜토리얼 개편 착수 지시문](IMPLEMENTATION-2026-07-31-tutorial-overhaul.md) | 완료 | M0~M6·R1~R7 완료. 채집 거점 필수 배정·촌 단계 채광장에 이어 길잡이 v7의 벌목장 터→건축가→완공 순서와 17단계 완주 회귀까지 갱신 |
