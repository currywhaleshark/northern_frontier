...
| [어장·어선·출어 설계](DESIGN-2026-08-02-fishing-boats.md) | 일부 완료 | F0~F4, F5 해안 이음매·염부·한선형 어선과 비직사각 복합 포구 완료. 일반 계절 포구채·축별 잔교·반전 없는 4방향 계류대를 게임/스튜디오 공용 manifest로 반영했으며 겨울 포구, 지형 전이 타일, 어살터·자염막 자산이 남음 (`test_fishing_port_assets.mjs`, F2~F4, 스프라이트 스튜디오 타입 검사, 빌드) |
| [새 게임 설정 개편 설계](DESIGN-2026-08-01-new-game-setup.md) | 일부 완료 | S0~S5 지역·크기 완료. 해수 비음용·비관개·비소방·비결빙과 남쪽 연속 해안·자염 체인을 core 83개·표적/자산 테스트·빌드·실화면으로 검증했고 세부설정 노브 소비처 연결이 남음 |
...
| [해안 전이 지형과 갯벌 어로 구현계획](IMPLEMENTATION-2026-08-03-coastal-transition-tidal-flat.md) | 완료 | 무지물·건축 금지 모래/갯벌, 첫 내륙 평지 자염막, `mudflat` 공동 비축·어살터·숲식 해안 이음매와 염부의 실제 해안 취수 왕복을 지형·UI·스프라이트 검사·빌드·실화면으로 검증 |
| [디버그 치트 패널 설계](DESIGN-2026-08-03-debug-cheat-panel.md) | 미착수 | 개발 빌드 한정 플로팅 패널 — 자원·시간·티어 조작, 주민/특수/가축 스폰, 사건 강제 발화(기존 발생 함수 버튼화), debugTouched 표식 |
| [밸런스 편집기 설계](DESIGN-2026-08-03-balance-editor.md) | 미착수 | B안(오버레이) 확정 — 별도 앱(`edit:balance`)이 변경분만 JSON+코드젠으로 반영, config 주석 동반 표시, 반영 시점 배지, 흡수 라운드 B1~B4 |
...
| [방어 개편 설계](DESIGN-2026-08-01-defense-overhaul.md) | 일부 완료 | P1~P4 완료. 벽·성문, 경로·다중 돌파·수리, 장기 공성·실이동 약탈·피난·3태세에 이어 망루 1인 주둔, 가시 화살, 첫 진입 보장 피해, 일일 상한·억제, 재표적·탈출과 v56 저장을 표적 회귀·빌드로 검증. P5 성벽 단면 전술 무대가 남음 |
