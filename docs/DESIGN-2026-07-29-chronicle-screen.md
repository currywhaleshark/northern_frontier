...
> **계획 상태:** 완료
> **상태 갱신:** 2026-07-29 — C1~C4 구현·검증 완료 (`72a4922`, `f12e560`).
> 세이브 스키마는 착수 시점 최신이 v42여서 **v43**으로 반영됐다.
> 문서 범위 외 추가: 행정단위 표기 — 저장은 밑이름만 담고 표기는 등급이 정한다
> (설한촌 → 설한보 → 설한진 → 설한부). 입력 꼬리의 단위 한 글자는 밑이름이 두 글자
> 이상 남을 때만 떼어 낸다. 상단 날짜줄 맨 왼쪽과 저장 슬롯 목록에 마을 표기 추가.
> 개칭 적용 연대기는 `special`이 아니라 `court`로 기록한다 (§1-1에 반영됨).
