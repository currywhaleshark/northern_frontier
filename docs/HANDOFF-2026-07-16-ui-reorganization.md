...
- UI 재구성 계획에서 유일하게 체크되지 않은 항목은 U1 자원 그룹의 시각 검증이다. 최초 검증 때 내장 브라우저의 localhost 연결이 막혀 보류됐다.
- 이후 내장 브라우저 연결은 정상화됐으므로 계획을 완전히 닫으려면 그룹 접힘, 팝오버, 별표 추가·제거, 숨은 품목 부족 강조만 실제 화면에서 확인하고 계획서 체크박스와 스크린샷을 갱신하면 된다.
- 전체 게임 테스트는 작은 UI 수정마다 반복하지 않았다. 변경 범위에 직접 관련된 테스트와 프로덕션 빌드를 우선 실행하는 현재 작업 방식을 유지한다.
