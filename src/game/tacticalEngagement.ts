...
export function resolveEngagementExchange(input: EngagementExchangeInput): EngagementExchangeResult {
  ...
  for (const defender of defenders) {
    ...
    if (wounded < active && input.rng() < clamp(fractionalRisk + risk * 0.2, 0, 0.9)) wounded += 1;
    let killed = enemyShare > 0.55 && input.rng() < risk * 0.32 ? 1 : 0;
  ...
  for (const attacker of attackers) {
    ...
    if (killed < activeRaiders && input.rng() < expectedKilled - killed) killed += 1;
    killed = Math.min(activeRaiders, killed);
    const powerAfter = Math.max(0, attacker.power * (1 - groupLossRate));
    raiderLosses.push({
      groupId: attacker.id,
      killed,
      powerAfter,
