...
export function newGameFromOptions(
  ...
  const { tiles, centerX, centerY } = generateMap(
    ...
    options.region,
    worldSetup.effective.resourceDensityMultiplier,
  );
  ...
  const state: GameState = {
    ...
    weather: 'clear',
    map: tiles,
    aquiferLevels: initialAquiferLevels(
      s, tiles[0]?.length ?? 0, tiles.length, options.region, effective.resourceDensityMultiplier),
    oreVeinRemaining: initialOreVeinRemaining(
      s, tiles[0]?.length ?? 0, tiles.length, options.region, effective.resourceDensityMultiplier),
  ...
  rng();
  state.weather = weatherForDay(s, 1, effective.climateSeverityMultiplier);
...
function endOfDay(state: GameState): void {
  ...
  rng();
  state.weather = weatherForDay(
    state.seed, state.day, state.worldSetup?.effective.climateSeverityMultiplier,
  );
