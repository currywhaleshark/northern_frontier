...
export function seaConditionForDay(
  seed: number,
  day: number,
  weather: WeatherId = weatherForDay(seed, day),
): SeaCondition {
  const climate = annualClimate(seed, getYear(day));
  const storminess = Number.isFinite(climate.storminess)
    ? Math.max(-1, Math.min(1, climate.storminess))
    : 0;
  const base = CONFIG.fishingBoats.seaConditionWeatherChance[weather];
...
export function seaConditionAt(
  ...
  return seaConditionForDay(state.seed, state.day, state.weather);
}

export function forecastSeaCondition(
  state: Pick<GameState, 'seed' | 'day'>,
  daysAhead = 1,
): { day: number; weather: WeatherId; condition: SeaCondition } {
  const day = Math.max(1, state.day + Math.max(0, Math.floor(daysAhead)));
  const weather = weatherForDay(state.seed, day);
  return { day, weather, condition: seaConditionForDay(state.seed, day, weather) };
}
