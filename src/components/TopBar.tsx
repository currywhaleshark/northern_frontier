...
function isLow(state: GameState, id: ResourceId, pop: number): boolean {
  if (id === 'food') return state.resources.food < pop * 3;
    ...
    <div className="topbar">
      <div className="topbar-row">
        {RESOURCE_ORDER.map(id => (
          <span key={id} className={`res-item${isLow(state, id, pop) ? ' low' : ''}`} title={RESOURCE_NAMES[id]}>
            {RESOURCE_ICONS[id]} <small>{RESOURCE_NAMES[id]}</small> {Math.floor(state.resources[id])}
          </span>
        ))}
      </div>
      <div className="topbar-row">
