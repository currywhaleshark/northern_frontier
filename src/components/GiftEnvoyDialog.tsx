...
import { eligibleGiftResources, giftPreview } from '../game/diplomacy';
...
export function GiftEnvoyDialog({ state, onSend, onClose }: Props) {
  const factionName = state.pendingChoice?.kind === 'giftEnvoy'
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal gift-envoy-dialog" onClick={event => event.stopPropagation()}>
        <h2>{leader ? `${leader.name} ${leader.title}에게 예물` : '예물 사절'}</h2>
        <div className="body">
          {leader && (
          ...
          <p>사치품과 은만 예물로 보낼 수 있습니다. 사절은 {CONFIG.diplomacy.envoyTravelDays}일 뒤 답신을 들고 돌아옵니다.</p>
          ...
          <div className="gift-envoy-preview">
            <span>예물 가치 {preview.value.toFixed(1)}</span>
            <strong>관계 +{preview.relationGain}</strong>
            <span>모반 의심 +{preview.suspicion}</span>
          </div>
          {preview.repeatedThisYear && <p className="gift-envoy-repeat">같은 해의 두 번째 예물이라 관계 상승폭이 절반입니다.</p>}
        </div>
        <div className="modal-actions">
          <button className="btn primary" disabled={blocked} title={blocked ? '보낼 예물이 없습니다' : undefined} onClick={() => onSend(resource, amount)}>
            사절을 보낸다
          </button>
